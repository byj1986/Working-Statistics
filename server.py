        ...
        try:
            ...
            if path == '/api/dates':
                try:
                    ...
                    logging.exception("Error in /api/dates")
            ...
            # API: 获取指定日期的数据
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        logging.exception("Error reading %s", file_path)
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json; charset=utf-8')
                        self.end_headers()
                        self.wfile.write(json.dumps({"error": str(e)}, ensure_ascii=False).encode('utf-8'))
            ...
            logging.exception("Unexpected error in do_GET")
            self.send_response(500)
            self.end_headers()
...
if __name__ == "__main__":
    ...
    try:
        ...
        with socketserver.TCPServer(("0.0.0.0", PORT), StatsHandler) as httpd:
            ...
            logging.info("%s", "=" * 60)
            logging.info("服务器已启动！")
            logging.info("本地访问: http://localhost:%s", PORT)
            logging.info("      或: http://127.0.0.1:%s", PORT)
            logging.info("局域网访问: http://%s:%s", socket.gethostbyname(socket.gethostname()), PORT)
            logging.info("访问页面:")
            logging.info("  - http://localhost:%s/daily.html", PORT)
            logging.info("  - http://localhost:%s/week.html", PORT)
            logging.info("API 接口:")
            logging.info("  - http://localhost:%s/api/dates", PORT)
            logging.info("  - http://localhost:%s/api/data/YYYYMMDD", PORT)
            logging.info("%s", "=" * 60)
            httpd.serve_forever()
    except OSError as e:
        if "Address already in use" in str(e) or "只允许使用一次" in str(e):
            logging.error("错误: 端口 %s 已被占用！", PORT)
            logging.info("请关闭占用该端口的程序，或修改 PORT 变量使用其他端口。")
            logging.info("检查占用端口的进程:")
            logging.info("  Windows: netstat -ano | findstr :%s", PORT)
            logging.info("  然后使用: taskkill /PID <进程ID> /F")
        else:
